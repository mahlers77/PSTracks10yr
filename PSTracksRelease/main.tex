...
While similar in goal and methods to ~\cite{IceCube:2019}, this sample includes several improvements. The angular reconstruction has been improved, and the event classifier and sample pre-cuts have been altered to better reject cascade-like events. These changes mean that some events present in previous data releases may not be present in this sample, even for overlapping time periods.
...
The lower significance of the 2014/2015 neutrino flare when performing the flare search with this data release appears to have been caused by the presence of two cascade-like events that existed in the data sample associated with ~\cite{IceCube:2019}, but have been removed from the data sample presented in this document. Performing the untriggered flare search using the sample associated with `\cite{IceCube:2019}, but with the two cascade events removed seems to produce a drop in significance similar to that which is observed when using this sample, as seen in table ~\ref{tab:TXSCrossChecks}. 
